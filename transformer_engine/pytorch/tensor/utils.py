...
from transformer_engine.musa.pytorch.tensor.mtfp8_tensor import MTFP8Quantizer, MTFP8Tensor
...
def replace_raw_data(tensor: QuantizedTensor, new_raw_data: torch.Tensor):
    ...
    if isinstance(tensor, Float8Tensor):
        ...
        del old_raw_data
    elif isinstance(tensor, MTFP8Tensor):
...
def cast_master_weights_to_fp8(
    ...
    for model_weight, master_weight, start_offset, fsdp_shard_model_weight in zip(
        ...
        elif isinstance(quantizer, MTFP8Quantizer):
...
def _cast_master_weights_to_fp8_blockwise_scaling(
    ...
    # Parameter attributes
    device = params[0][0].device
    block_len = params[0][0]._quantizer.block_m
    fp8_dtype = params[0][0]._get_quantizer().dtype
    force_pow_2_scales = False # params[0][0]._get_quantizer().force_pow_2_scales
    amax_epsilon = 1e-4 # params[0][0]._get_quantizer().amax_epsilon
