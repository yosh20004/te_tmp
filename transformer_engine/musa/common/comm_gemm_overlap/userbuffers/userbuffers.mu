...
#define UB_PRINT(message, ...) \
  printf("[%s:%s:%d] " message "\n", __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__) // FIXME(yehua.zhang): FILENAME(__FILE__) compile err because of mcc
...
template <int RANKS, typename HALF_TYPE>
__global__ void __launch_bounds__(MAX_THREADS)
...
#pragma unroll
    ...
    int4 sum = val[0];
    HALF_TYPE *s = reinterpret_cast<HALF_TYPE *>(&sum);

#pragma unroll
    for (int i = 1; i < RANKS; i++) {
        HALF_TYPE *x = reinterpret_cast<HALF_TYPE *>(&val[i]);
#pragma unroll
      for (int j = 0; j < 8; j++) s[j] += x[j];
    }
...
//   musaLaunchAttribute attribute_ub[2];                                               \
//   attribute_ub[1].id = musaLaunchAttributeClusterDimension;                          \
...
//   attribute_ub[0].id = musaLaunchAttributeCooperative;                               \
//   cfg.attrs = attribute_ub;                                                          \
//   cfg.numAttrs = comm->sm_arch >= 9 ? 2 : 1;

// TODO(yuzhe.wu): Temporarily disable AttributeCooperative and
// enable it again when the driver supports musaLaunchAttributeCooperative.
#define SETUP_LAUNCH_CONFIG(sms, threads, stream)                                    \
  musaLaunchConfig_t cfg = {sms, threads, 0, stream, NULL, 0};                       \
  cfg.numAttrs = 0;
...
#define SETUP_LAUNCH_CONFIG_WITH_COMPLETION_EVENT(sms, threads, stream, comm_launch_event) \
  musaLaunchConfig_t cfg = {sms, threads, 0, stream, NULL, 0};                             \
  musaLaunchAttribute attribute_ub[NUM_LAUNCH_ATTRIBUTE_FOR_FDL_LAUNCH] = {};              \
...
#define callranks_rsMC(x)                                                                        \
  ...
  #define callranks_rs_oop(x, is_bf16)                                                                   \
  if (ar_nvsize == x) {                                                                       \
    int arg1 = op - NVTE_MAX_OPS,                                                             \
        arg2 = NVTE_REG0_OFFSET(comm) -                                                       \
               (op == userbuffers_allreduceop_nonsharp ? 2 : 1) * NVTE_REG0_SINGLENODE +      \
...
void reducescatter2_userbuff_strided_universal_fp8(void *output, float *scale, const int handler,
                                                   ...
                                                   communicator *comm, musaStream_t stream) {
  const int elements = rowelements * colelements;
  const int op = userbuffers_allreduceop_nonsharp2;
  const int ar_firstgpu =
      op == userbuffers_allreduceop_nonsharp ? comm->ar_firstgpu : comm->ar2_firstgpu;
...
void reducescatter2_userbuff_strided_multiatomic_fp8(
    ...
    const int numchunks, void *counters, communicator *comm, musaStream_t stream) {
  reducescatter2_userbuff_strided_universal_fp8<fp8type>(
      output, scale, handler, offset, rowelements, colelements, strideelements_out,
...
void reducescatter2_userbuff_stridedoutput(void *output, transformer_engine::DType dtype,
                                           ...
                                           const int rowelements, const int colelements,
                                           const int strideelements, communicator *comm,
                                           musaStream_t stream, musaEvent_t comm_launch_event) {
  const int elements = rowelements * colelements;
  const int op = userbuffers_allreduceop_nonsharp2;
  const int ar_firstgpu =
      op == userbuffers_allreduceop_nonsharp ? comm->ar_firstgpu : comm->ar2_firstgpu;
...
void reducescatter2_userbuff(void *output, transformer_engine::DType dtype, const int handler, const int offset, const int elements,
                             communicator *comm, musaStream_t stream,
                             musaEvent_t comm_launch_event) {
  reducescatter2_userbuff_stridedoutput(output, dtype, handler, offset, elements, 1, 0, comm, stream,
                                        comm_launch_event);
}
...
template void reducescatter2_userbuff_stridedoutput_fp8<__mt_fp8_e5m2>(
    void *output, float *scale, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements, communicator *comm, musaStream_t stream,
    musaEvent_t comm_launch_event);

template void reducescatter2_userbuff_stridedoutput_fp8<__mt_fp8_e4m3>(
    void *output, float *scale, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements, communicator *comm, musaStream_t stream,
    musaEvent_t comm_launch_event);
...
void reducescatter2_userbuff_fp8(void *output, float *scale, const int handler, const int offset,
                                 const int elements, communicator *comm, musaStream_t stream,
                                 musaEvent_t comm_launch_event) {
  reducescatter2_userbuff_stridedoutput_fp8<fp8type>(output, scale, handler, offset, elements, 1, 0,
                                                     comm, stream, comm_launch_event);
}

template void reducescatter2_userbuff_fp8<__mt_fp8_e5m2>(void *output, float *scale,
                                                         const int handler, const int offset,
                                                         const int elements, communicator *comm,
                                                         musaStream_t stream,
                                                         musaEvent_t comm_launch_event);
template void reducescatter2_userbuff_fp8<__mt_fp8_e4m3>(void *output, float *scale,
                                                         const int handler, const int offset,
                                                         const int elements, communicator *comm,
                                                         musaStream_t stream,
                                                         musaEvent_t comm_launch_event);

template void reducescatter2_userbuff_strided_atomic_fp8<__mt_fp8_e4m3>(
    void *output, float *scale, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements_out, const int strideelements_in,
    const int numchunks, void *counters, communicator *comm, musaStream_t stream);
template void reducescatter2_userbuff_strided_atomic_fp8<__mt_fp8_e5m2>(
    void *output, float *scale, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements_out, const int strideelements_in,
    const int numchunks, void *counters, communicator *comm, musaStream_t stream);
template void reducescatter2_userbuff_strided_multiatomic_fp8<__mt_fp8_e4m3>(
    void *output, float *scale, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements_out, const int strideelements_in,
    const int numchunks, void *counters, communicator *comm, musaStream_t stream);
template void reducescatter2_userbuff_strided_multiatomic_fp8<__mt_fp8_e5m2>(
    void *output, float *scale, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements_out, const int strideelements_in,
    const int numchunks, void *counters, communicator *comm, musaStream_t stream);
...
__global__ void kuserbuffers_pullsend(int myrank, int peer, int *send_id, int *flagptr) {
  ...
  // TODO(xiaoyang): replace atomicAdd_system temporarily
  atomicAdd(flagptr, 1);
}
...
void userbuffers_send(const int srchandler, const size_t srcoffset, const int dsthandler,
                      ...
                      const int peer, musaStream_t stream) {
...
void userbuffers_sendrecv(const int srchandler, const int dsthandler, const size_t send_offset,
                          ...
                          const int send_peer, const int recv_peer, musaStream_t stream) {
  bool signalonly = (bytes / 16 == 0) || (comm->use_ce != 0);
  int send_peerlocal = send_peer % comm->nvsize;
  int recv_peerlocal = recv_peer % comm->nvsize;
  void *flagptr_send = GET_SEND_PTR_BY_INDEX(send_peerlocal, comm, dsthandler, 0);
...
void userbuffers_sendrecv_atomic(const int srchandler, const int dsthandler,
                                 ...
                                 const int recv_peer, void *counters, musaStream_t stream) {
...
void userbuffers_sendrecv_multiatomic(const int srchandler, const int dsthandler,
                                      ...
                                      bool shuffle, musaStream_t stream) {
  assert(comm->push && comm->use_ce == 0);
  // CE is not supported
...
__global__ void __launch_bounds__(MAX_THREADS / 4)
    reduce_fp8_in_bf16_out_musa(void *inputs, void *output, const float *scale,
...
void reduce_fp8_in_bf16_out(void *inputs, void *output, float *scale, int num_inputs,
                            int input_size, musaStream_t stream) {
  size_t num_threads = MAX_THREADS / 4;
  size_t num_blocks = (input_size + num_threads - 1) / num_threads;
  dim3 block(num_threads);
  dim3 grid(num_blocks);
  reduce_fp8_in_bf16_out_musa<fp8type>
      <<<grid, block, 0, stream>>>(inputs, output, scale, num_inputs, input_size);
}

template void reduce_fp8_in_bf16_out<__mt_fp8_e4m3>(void *inputs, void *output, float *scale,
                                                    int num_inputs, int input_size,
                                                    musaStream_t stream);
template void reduce_fp8_in_bf16_out<__mt_fp8_e5m2>(void *inputs, void *output, float *scale,
                                                    int num_inputs, int input_size,
                                                    musaStream_t stream);
...
void reduce_bf16(void *inputs, void *output, int num_inputs, int input_size, musaStream_t stream) {
  ...
  dim3 block(num_threads);
  dim3 grid(num_blocks);
  reduce_bf16_musa<<<grid, block, 0, stream>>>(inputs, output, num_inputs, input_size);
}
