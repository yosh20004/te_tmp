...
int create_communicator_grouped2(communicator **comm, int myrank, int numranks, int mylocal,
  ...
  *comm = new communicator();
  ...
  NVTE_CHECK_CUDA(musaGetDevice(&cur_dev));
  NVTE_CHECK_CUDA(musaGetDeviceCount(&ndev));
  NVTE_CHECK_CUDA(musaGetDeviceProperties(&device_prop, cur_dev));
  ...
  NVTE_CHECK_CUDA(musaDeviceGetAttribute(&device_clock, musaDevAttrClockRate, cur_dev));
  ...
  if (ndev == numlocal) {  // all visible devices
    ...
    NVTE_CHECK_CUDA(musaSetDevice(mylocal));
  }
  (*comm)->mydev = cur_dev;
...
#define NBUF 2
  ...
  if (!transformer_engine::getenv<bool>("UB_SKIPMC") &&
    ...
    NVTE_CALL_CHECK_CUDA_DRIVER(
        muMulticastGetGranularity, &gran, &mcProp,
    ...
    //       file descriptor and prevent muMemImportFromShareableHandle() from correctly
    ...
    if ((*comm)->ar2_nvrank == 0) {
      NVTE_CALL_CHECK_CUDA_DRIVER(muMulticastCreate, &(*comm)->mc_handle, &mcProp);
      NVTE_CALL_CHECK_CUDA_DRIVER(
          muMemExportToShareableHandle, reinterpret_cast<void *>(&fd), (*comm)->mc_handle,
  ...
  error:
    if ((*comm)->ar2_nvrank != 0) {
      NVTE_CALL_CHECK_CUDA_DRIVER(
          muMemImportFromShareableHandle, &(*comm)->mc_handle, reinterpret_cast<void *>(fd),
          static_cast<CUmemAllocationHandleType>(CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR));
    }
    IPCCHECK(ipcSocketClose(&ipcSock));
    close(fd);
    NVTE_CALL_CHECK_CUDA_DRIVER(muMulticastAddDevice, (*comm)->mc_handle,
                                (MUdeviceptr)(*comm)->mydev);

    MUdeviceptr mc_va;
    NVTE_CALL_CHECK_CUDA_DRIVER(muMemAddressReserve, &mc_va, mc_maxsize, (size_t)0, (MUdeviceptr)0U,
                                (uint64_t)0);
    NVTE_CALL_CHECK_CUDA_DRIVER(muMemMap, mc_va, mc_maxsize, (size_t)0, (*comm)->mc_handle,
                                (uint64_t)0);
    ...
    NVTE_CALL_CHECK_CUDA_DRIVER(muMemSetAccess, mc_va, mc_maxsize,
...
#define LOCALSIZE 4 * (NVTE_REG0_OFFSET(*comm) + NVTE_REG0_FLAGS + NVTE_REG0_COMMBUFFER * NBUF)
  ...
  NVTE_CHECK_CUDA(musaDeviceSynchronize());
  register_user_buffer_collective(&((*comm)->gpu_ptrs), LOCALSIZE, *comm, true);
  NVTE_CHECK_CUDA(musaMalloc(&(*comm)->send_id, (*comm)->nranks * sizeof(int)));
  NVTE_CHECK_CUDA(musaMalloc(&(*comm)->recv_id, NVTE_MAX_REGIONS * (*comm)->nranks * sizeof(int)));
  NVTE_CHECK_CUDA(musaMemset((*comm)->send_id, 0, (*comm)->nranks * sizeof(int)));
  NVTE_CHECK_CUDA(
      musaMemset((*comm)->recv_id, 0, NVTE_MAX_REGIONS * (*comm)->nranks * sizeof(int)));
  (*comm)->sms = 16;
  (*comm)->threads = 1024;
...
#define GPU_PAGE_MASK (~GPU_PAGE_OFFSET)
  ...
  NVTE_CHECK_CUDA(musaMalloc(&(*comm)->flags, 2 * GPU_PAGE_SIZE));
  NVTE_CHECK_CUDA(musaMemset((*comm)->flags, 0, 2 * GPU_PAGE_SIZE));
  (*comm)->flags =
      reinterpret_cast<int *>(((MUdeviceptr)(*comm)->flags + GPU_PAGE_SIZE - 1) & GPU_PAGE_MASK);
...
void destroy_communicator(communicator *comm) {
  for (int hndl = 0; hndl < comm->free_region; hndl++) {
    if (comm->use_mc && comm->mem_dealloc[hndl]) {
      for (int rank = 0; rank < comm->nvsize; rank++) {
        if (rank == comm->nvrank) {
          NVTE_CALL_CHECK_CUDA_DRIVER(muMemRelease, comm->uchandles[hndl][rank]);
        } else {
      ...
      for (int rank = 0; rank < comm->nvsize; rank++) {
        if (rank != comm->nvrank) {
          musaIpcCloseMemHandle(comm->peer_ptr[hndl][rank]);
        } else if (comm->mem_dealloc[hndl]) {
          NVTE_CHECK_CUDA(musaFree(comm->peer_ptr[hndl][rank]));
        } else {
  ...
  musaFree(reinterpret_cast<void *>(comm->recv_id));
  musaFree(reinterpret_cast<void *>(comm->send_id));
  if (comm->use_mc) {
...
int register_user_buffer_collective(void **gpubuff, size_t bytes, communicator *comm, bool alloc) {
  ...
  if (comm->use_mc && alloc) {
    ...
    NVTE_CALL_CHECK_CUDA_DRIVER(
        muMemGetAllocationGranularity, &granularity, &prop,
    ...
    if (comm->use_mc) {
      ...
      NVTE_CALL_CHECK_CUDA_DRIVER(
          muMulticastGetGranularity, &granularity, &mcProp,
          static_cast<CUmemAllocationGranularity_flags>(CU_MULTICAST_GRANULARITY_MINIMUM));
    ...
    NVTE_CALL_CHECK_CUDA_DRIVER(muMemCreate, &(comm->uchandles[hndl][myrank]), aligned_size, &prop,
                                (uint64_t)0);

    int *peerfd = reinterpret_cast<int *>(malloc(nranks * sizeof(int)));
    NVTE_CALL_CHECK_CUDA_DRIVER(
        muMemExportToShareableHandle, reinterpret_cast<void *>(&peerfd[myrank]),
  ...
  error:
    ...
    for (int p = 0; p < nranks; p++) {
      if (p != myrank)
        NVTE_CALL_CHECK_CUDA_DRIVER(
            muMemImportFromShareableHandle, &comm->uchandles[hndl][p],
    ...
    MUdeviceptr ptr;
    NVTE_CALL_CHECK_CUDA_DRIVER(muMemAddressReserve, &ptr, (size_t)(aligned_size * nranks),
                                (size_t)0, (MUdeviceptr)0, (uint64_t)0);
    ...
    for (int i = 0; i < nranks; i++) {
      ...
      NVTE_CALL_CHECK_CUDA_DRIVER(muMemMap, reinterpret_cast<MUdeviceptr>(remptrs[i]), aligned_size,
    ...
    NVTE_CALL_CHECK_CUDA_DRIVER(muMemSetAccess, ptr, (size_t)(aligned_size * nranks),
                                const_cast<CUmemAccessDesc *>(&accessDesc), (size_t)1);

    if (hndl == 0) NVTE_CHECK_CUDA(musaMemset(comm->gpu_ptrs, 0, aligned_size));
    NVTE_CHECK_CUDA(
        musaMemcpy((reinterpret_cast<char *>(comm->gpu_ptrs)) + (hndl * nranks * sizeof(void *)),
                   remptrs, nranks * sizeof(void *), musaMemcpyHostToDevice));
    free(remptrs);
    comm->memflags[hndl] = UB_MEM_UC_CONTIG | UB_MEM_ALLOCATED;

    if (comm->use_mc && comm->mc_maxsize >= comm->mc_offset + aligned_size) {
      NVTE_CALL_CHECK_CUDA_DRIVER(muMulticastBindMem, comm->mc_handle, comm->mc_offset,
...
#endif
    if (alloc) {
      NVTE_CHECK_CUDA(musaMalloc(gpubuff, bytes));
      NVTE_CHECK_CUDA(musaMemset(*gpubuff, 0, bytes));
    }

    NVTE_CHECK(comm->nvsize <= 8, "CUDA IPC supports only up to 8 GPUs in an NVLink domain.");
    musaIpcMemHandle_t memhndl;
    NVTE_CHECK_CUDA(musaIpcGetMemHandle(&memhndl, *gpubuff));

    musaIpcMemHandle_t *tmp =
        reinterpret_cast<musaIpcMemHandle_t *>(malloc(comm->nvsize * sizeof(musaIpcMemHandle_t)));
    comm->_allgather(reinterpret_cast<void *>(tmp), comm->nvsize * sizeof(musaIpcMemHandle_t),
                     reinterpret_cast<void *>(&memhndl), sizeof(musaIpcMemHandle_t),
                     comm->comm_intra);

    for (int i = 0; i < comm->nvsize; i++) {
      if (i != comm->nvrank) {
        NVTE_CHECK_CUDA(musaIpcOpenMemHandle(&(comm->peer_ptr[hndl][i]), tmp[i],  // NOLINT(*)
    ...
    NVTE_CHECK_CUDA(musaMemcpy(
        reinterpret_cast<char *>(comm->gpu_ptrs) + (hndl * comm->nvsize * sizeof(void *)),
        comm->peer_ptr[hndl], comm->nvsize * sizeof(void *), musaMemcpyHostToDevice));
