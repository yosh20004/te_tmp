...
class Kernel {
 public:
  ...
  void launch(int device_id, const dim3 grid_dim, const dim3 block_dim,
              unsigned int shared_mem_bytes, musaStream_t stream, ArgTs &&...args) {
    void *arg_ptrs[] = {const_cast<void *>(static_cast<const void *>(&args))...};
    NVTE_CALL_CHECK_CUDA_DRIVER(muLaunchKernel, get_function(device_id), grid_dim.x, grid_dim.y,
                                grid_dim.z, block_dim.x, block_dim.y, block_dim.z, shared_mem_bytes,
                                static_cast<MUstream>(stream), arg_ptrs, nullptr);
  }
 ...
 private:
  ...
  std::vector<MUmodule> modules_;
  /*! CUDA function for each CUDA device */
  std::vector<MUfunction> functions_;
  ...
  static constexpr MUmodule null_module = static_cast<MUmodule>(nullptr);
  /*! Uninitialized CUDA function */
  static constexpr MUfunction null_function = static_cast<MUfunction>(nullptr);
};
...
class KernelManager {
 public:
   ...
   */
  void set_cache_config(const std::string &kernel_label, MUfunc_cache cache_config);
