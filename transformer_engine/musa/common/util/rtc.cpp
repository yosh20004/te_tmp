...
#include "string_code_utils_muh.h"
...
Kernel::~Kernel() {
  for (int device_id = 0; device_id < static_cast<int>(modules_.size()); ++device_id) {
    ...
    if (modules_[device_id] != null_module) {
      MUdevice device;
      MUcontext context;
      if (cuda_driver::call("muDeviceGet", &device, device_id) != MUSA_SUCCESS) {
        continue;
      }
      if (cuda_driver::call("muDevicePrimaryCtxRetain", &context, device) != MUSA_SUCCESS) {
        continue;
      }
      if (cuda_driver::call("muCtxSetCurrent", context) != MUSA_SUCCESS) {
        continue;
      }
      cuda_driver::call("muModuleUnload", modules_[device_id]);
      cuda_driver::call("muDevicePrimaryCtxRelease", device);
    }
  }
}
...
MUfunction Kernel::get_function(int device_id) {
  // Load kernel on device if needed
  auto load_on_device = [&]() {
    // Set driver context to proper device
    MUdevice device;
    MUcontext context;
    NVTE_CALL_CHECK_CUDA_DRIVER(muDeviceGet, &device, device_id);
    NVTE_CALL_CHECK_CUDA_DRIVER(muDevicePrimaryCtxRetain, &context, device);
    NVTE_CALL_CHECK_CUDA_DRIVER(muCtxSetCurrent, context);

    // Load function into driver context
    NVTE_CALL_CHECK_CUDA_DRIVER(muModuleLoadDataEx, &modules_[device_id], compiled_code_.c_str(),
                                0,         // numOptions
                                nullptr,   // options
                                nullptr);  // optionValues
    NVTE_CALL_CHECK_CUDA_DRIVER(muModuleGetFunction, &functions_[device_id], modules_[device_id],
                                mangled_name_.c_str());

    // Reset driver context
    NVTE_CALL_CHECK_CUDA_DRIVER(muDevicePrimaryCtxRelease, device);
  };
...
void Kernel::set_function_cache_config(int device_id, MUfunc_cache cache_config) {
  NVTE_CALL_CHECK_CUDA_DRIVER(muFuncSetCacheConfig, get_function(device_id), cache_config);
}
...
//   constexpr const char* headers[num_headers] = {string_code_utils_muh, string_code_util_math_h};
//   constexpr const char* include_names[num_headers] = {"utils.muh", "util/math.h"};
...
void KernelManager::set_cache_config(const std::string& kernel_label, MUfunc_cache cache_config) {
  const int device_id = cuda::current_device();
  const auto key = get_kernel_cache_key(kernel_label, device_id);
