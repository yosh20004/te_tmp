...
inline MUresult call(const char *symbol, ArgTs... args) {
  using FuncT = MUresult(ArgTs...);
...
#define NVTE_CHECK_CUDA_DRIVER(expr)                                                             \
  do {                                                                                           \
    const MUresult status_NVTE_CHECK_MUSA_DRIVER = (expr);                                       \
    if (status_NVTE_CHECK_MUSA_DRIVER != MUSA_SUCCESS) {                                         \
      const char *desc_NVTE_CHECK_MUSA_DRIVER;                                                   \
      ::transformer_engine::cuda_driver::call("muGetErrorString", status_NVTE_CHECK_MUSA_DRIVER, \
                                              &desc_NVTE_CHECK_MUSA_DRIVER);                     \
