...
#include "string_path_musa_include.h"
...
int num_devices() {
  auto query_num_devices = []() -> int {
    int count;
    NVTE_CHECK_CUDA(musaGetDeviceCount(&count));
    return count;
  };
...
int current_device() {
  ...
  MUcontext context;
  NVTE_CALL_CHECK_CUDA_DRIVER(muCtxGetCurrent, &context);
  if (context == nullptr) {
    return 0;
  }
  ...
  int device_id;
  NVTE_CHECK_CUDA(musaGetDevice(&device_id));
  return device_id;
}
...
int sm_arch(int device_id) {
  ...
  NVTE_CHECK(0 <= device_id && device_id < num_devices(), "invalid MUSA device ID");
  auto init = [&]() {
    musaDeviceProp prop;
    NVTE_CHECK_CUDA(musaGetDeviceProperties(&prop, device_id));
...
void stream_priority_range(int *low_priority, int *high_priority, int device_id) {
  ...
  auto init = [&]() {
    ...
    if (device_id != ori_dev) NVTE_CHECK_CUDA(musaSetDevice(device_id));
    int min_pri, max_pri;
    NVTE_CHECK_CUDA(musaDeviceGetStreamPriorityRange(&min_pri, &max_pri));
    if (device_id != ori_dev) NVTE_CHECK_CUDA(musaSetDevice(ori_dev));
...
const std::string &include_directory(bool required) {
  ...
  if (need_to_check_env) {
    ...
    std::vector<std::pair<std::string, Path>> search_paths = {{"NVTE_MUSA_INCLUDE_DIR", ""},
                                                              {"MUSA_HOME", ""},
                                                              {"MUSA_DIR", ""},
    ...
    for (auto &[env, p] : search_paths) {
      ...
      if (!p.empty()) {
        if (file_exists(p / "musa_runtime.h")) {
          path = p;
          break;
        }
        if (file_exists(p / "include" / "musa_runtime.h")) {
          path = p / "include";
          break;
        }
      }
    ...
    if (path.empty() && required) {
      ...
      message += "Could not find musa_runtime.h in";
      bool is_first = true;
      ...
      message +=
          (". "
           "Specify path to MUSA Toolkit headers "
           "with NVTE_MUSA_INCLUDE_DIR.");
      NVTE_ERROR(message);
    }
