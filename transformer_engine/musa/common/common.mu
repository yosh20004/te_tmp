...
void checkCuDriverContext(MUstream stream) {
  MUcontext ctx;
  const MUresult driver_status = cuda_driver::call("muStreamGetCtx", stream, &ctx);
  switch (driver_status) {
    ...
    case MUSA_ERROR_INVALID_CONTEXT:
      int current_device;
      NVTE_CHECK_CUDA(musaGetDevice(&current_device));
      NVTE_CALL_CHECK_CUDA_DRIVER(muDevicePrimaryCtxRetain, &ctx, current_device);
      NVTE_CALL_CHECK_CUDA_DRIVER(muCtxSetCurrent, ctx);
      break;

    default:
      const char *desc_NVTE_CHECK_MUSA_DRIVER;
      cuda_driver::call("muGetErrorString", driver_status, &desc_NVTE_CHECK_MUSA_DRIVER);
      NVTE_ERROR("MUSA Error: ", desc_NVTE_CHECK_MUSA_DRIVER);
  }
}
