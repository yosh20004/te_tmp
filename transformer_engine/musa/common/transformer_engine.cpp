...
void nvte_zero_tensor(const NVTETensor tensor, musaStream_t stream) {
  const auto &t = *reinterpret_cast<const transformer_engine::Tensor *>(tensor);
  // Zero out tensor data if allocated
  if (t.data.dptr != nullptr) {
    size_t size_in_bytes = nvte_tensor_element_size(tensor) * nvte_tensor_numel(tensor);
    musaMemsetAsync(t.data.dptr, 0, size_in_bytes, stream);
  }
  // Set amax to 0 if allocated
  if (t.amax.dptr != nullptr) {
    float zero = 0.0f;
    musaMemcpyAsync(t.amax.dptr, &zero, sizeof(float), musaMemcpyHostToDevice, stream);
  }
