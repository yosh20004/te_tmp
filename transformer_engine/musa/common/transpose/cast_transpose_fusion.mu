...
#include "string_code_transpose_rtc_cast_transpose_fusion_mu.h"
...
void reduce_dbias(const Tensor &workspace, Tensor *dbias, const size_t row_length,
                  const size_t num_rows, const int nvec_out, musaStream_t stream) {
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          ...
          if (jit_compiled) {
            ...
            if (!rtc_manager.is_compiled(kernel_label)) {
              std::string code = string_code_transpose_rtc_cast_transpose_fusion_mu;
              ...
              rtc_manager.compile(
                  ...
                  "transformer_engine/common/transpose/rtc/cast_transpose_fusion.mu");
            }

            rtc_manager.set_cache_config(kernel_label, MU_FUNC_CACHE_PREFER_SHARED);
            ...
            musaFuncSetAttribute(
                ...
                musaFuncAttributePreferredSharedMemoryCarveout, 100);
...
void nvte_cast_transpose_dbias(const NVTETensor input, NVTETensor output, NVTETensor dbias,
                               NVTETensor workspace, musaStream_t stream) {
...
void nvte_dgeglu_cast_transpose(const NVTETensor input, const NVTETensor gated_act_input,
                                NVTETensor output, musaStream_t stream) {
