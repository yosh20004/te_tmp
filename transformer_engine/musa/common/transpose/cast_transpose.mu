...
#include "string_code_transpose_rtc_cast_transpose_mu.h"
...
void cast_transpose(const Tensor &input, const Tensor &noop, Tensor *output_, musaStream_t stream) {
          ...
          if (is_delayed_tensor_scaling(output.scaling_mode)) {
            ...
            if (aligned && rtc::is_enabled()) {  // Runtime-compiled tuned kernel
              ...
              if (!rtc_manager.is_compiled(kernel_label)) {
                std::string code = string_code_transpose_rtc_cast_transpose_mu;
                ...
                rtc_manager.compile(kernel_label, "cast_transpose_optimized_kernel", code,
                                    "transformer_engine/common/transpose/rtc/cast_transpose.mu");
              }
...
void nvte_cast_transpose(const NVTETensor input, NVTETensor output, musaStream_t stream) {
