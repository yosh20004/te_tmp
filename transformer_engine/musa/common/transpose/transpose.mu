...
#include "string_code_transpose_rtc_transpose_mu.h"
...
void transpose(const Tensor &input, const Tensor &noop, Tensor *output_, musaStream_t stream) {
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
      ...
      if (aligned && rtc::is_enabled()) {  // Runtime-compiled tuned kernel
        ...
        if (!rtc_manager.is_compiled(kernel_label)) {
          std::string code = string_code_transpose_rtc_transpose_mu;
          ...
          rtc_manager.compile(kernel_label, "transpose_optimized_kernel", code,
                              "transformer_engine/common/transpose/rtc/transpose.mu");
        }
...
void nvte_transpose(const NVTETensor input, NVTETensor output, musaStream_t stream) {
