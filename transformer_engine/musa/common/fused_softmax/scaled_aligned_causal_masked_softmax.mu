...
void call_kernel_scaled_aligned_causal_masked_softmax_forward(
    dim3 grid_size, dim3 block_size, const int shmem_size, musaStream_t stream, output_t *dst,
...
void call_kernel_scaled_aligned_causal_masked_softmax_backward(
    dim3 grid_size, dim3 block_size, const int shmem_size, musaStream_t stream, output_t *gradInput,
...
struct FunctionWrapper {
  using ForwardType =
      std::function<void(dim3 grid_size, dim3 block_size, const int shmem_size, musaStream_t stream,
  ...
  using BackwardType = std::function<void(
      dim3 grid_size, dim3 block_size, const int shmem_size, musaStream_t stream,
...
void dispatch_scaled_aligned_causal_masked_softmax_forward(output_t *dst, const input_t *src,
                                                           ...
                                                           int attn_heads, musaStream_t stream) {
...
void dispatch_scaled_aligned_causal_masked_softmax_backward(
    ...
    int query_seq_len, int key_seq_len, int batches, int attn_heads, musaStream_t stream) {
  NVTE_CHECK(key_seq_len >= 0 && key_seq_len <= 16384, "Unsupported shape.");
...
void scaled_aligned_causal_masked_softmax_forward(const Tensor &input, Tensor *softmax_results,
                                                  float scale_factor, musaStream_t stream) {
