...
struct Tensor {
  ...
  std::vector<size_t> shape() const {
    switch (scaling_mode) {
      ...
      default:
        NVTE_ERROR("Cannot parse tensor shape with scaling mode \"", (int)scaling_mode, "\"");
        return {};
    }
  }
*/
...
using fp32 = float;
using fp16 = __half;
using bf16 = __mt_bfloat16;
using fp8e4m3 = __mt_fp8_e4m3;
using fp8e5m2 = __mt_fp8_e5m2;
...
TRANSFORMER_ENGINE_TYPE_NAME(__half)
TRANSFORMER_ENGINE_TYPE_NAME(__mt_bfloat16)
TRANSFORMER_ENGINE_TYPE_NAME(__mt_fp8_e4m3)
TRANSFORMER_ENGINE_TYPE_NAME(__mt_fp8_e5m2)
#if CUDA_VERSION >= 12080
...
inline bool is_mtfp_scaling(const NVTEScalingMode &mode) { return mode == NVTE_MTFP8_BLOCK_SCALING; }
 ...
 */
void update_tensor_scale_inv(Tensor *t, musaStream_t stream);

#define NVTE_API_CALL(api_name) \
  transformer_engine::nvtx::NVTXWrapper _##api_name##_nvtx_wrapper(#api_name);

void checkCuDriverContext(MUstream stream);
