...
at::Tensor allocateSpace(const std::vector<size_t>& shape, const transformer_engine::DType type,
  ...
  const auto opt = at::TensorOptions().dtype(GetATenDType(type))
      .device(
          c10::DeviceType::PrivateUse1,
          static_cast<c10::DeviceIndex>(transformer_engine::cuda::current_device()));
  if (init_to_zeros) {
    return at::zeros(ar_shape, opt);
  } else {
    return at::empty(ar_shape, opt);
  }
}
...
at::Tensor allocateSpace(const NVTEShape& shape, const transformer_engine::DType type,
  ...
  if (size == 2 && init_to_zeros) {
    return at::zeros({static_cast<int64_t>(shape.data[0]), static_cast<int64_t>(shape.data[1])}, opt);
  } else if (size == 2) {
    return at::empty({static_cast<int64_t>(shape.data[0]), static_cast<int64_t>(shape.data[1])}, opt);
  } else if (size == 1 && init_to_zeros) {
    return at::zeros({static_cast<int64_t>(shape.data[0])}, opt);
  } else if (size == 1) {
    return at::empty({static_cast<int64_t>(shape.data[0])}, opt);
  }
...
at::Tensor allocateTorchTensor(int M, int N, transformer_engine::DType dtype) {
  const auto opt = at::TensorOptions().dtype(GetATenDType(dtype))
      .device(
          c10::DeviceType::PrivateUse1,
          static_cast<c10::DeviceIndex>(transformer_engine::cuda::current_device()));
  return at::empty({static_cast<int64_t>(M), static_cast<int64_t>(N)}, opt);
}
...
at::Tensor allocateTorchTensor(int M, transformer_engine::DType dtype) {
  const auto opt = at::TensorOptions().dtype(GetATenDType(dtype))
      .device(
          c10::DeviceType::PrivateUse1,
          static_cast<c10::DeviceIndex>(transformer_engine::cuda::current_device()));
  return at::empty({static_cast<int64_t>(M)}, opt);
}
