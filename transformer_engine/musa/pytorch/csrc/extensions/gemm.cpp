...
std::vector<py::object> gemm(py::handle A, bool transa, py::handle B, bool transb, py::object D,
  ...
  if (bias.has_value()) {
    if (grad) {
      auto opts = torch::TensorOptions().dtype(GetATenDType(D_tensor.dtype())).device(c10::kPrivateUse1);
  ...
  if (gelu) {
    if (!grad) {
      ...
      auto opts = torch::TensorOptions().dtype(dtype).device(c10::kPrivateUse1);
  ...
  const int device_id = c10::musa::current_device();
  const int sm_count = transformer_engine::cuda::sm_count(device_id);
  int num_math_sms = sm_count - transformer_engine::getenv<int>("NVTE_EXT_MARGIN_SM", sm_count);

  auto main_stream = at::musa::getCurrentMUSAStream();
...
void te_atomic_gemm(at::Tensor A, at::Tensor A_scale_inverse, transformer_engine::DType A_type,
  ...
  nvte_cublas_atomic_gemm(te_A.data(), te_B.data(), te_D.data(), te_bias.data(),
                          ...
                          gemm_producer, te_counter.data(), at::musa::getCurrentMUSAStream());
}
...
std::optional<std::vector<at::Tensor>> te_general_grouped_gemm(
  ...
  for (size_t i = 0; i < A.size(); i++) {
    ...
    if (te_A.numel() == 0 || te_B.numel() == 0) {
      ...
      && std::string(std::getenv("TE_MULTI_STREAM_GROUPGEMM")) == "1") continue;
    }
