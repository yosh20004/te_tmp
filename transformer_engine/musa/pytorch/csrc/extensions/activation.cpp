...
template <void (*act_func)(const NVTETensor, NVTETensor, musaStream_t)>
py::object activation_helper(const at::Tensor& input, py::handle quantizer, int shape_divisor = 1) {
  ...
  act_func(te_input.data(), te_output.data(), at::musa::getCurrentMUSAStream());

  return out;
}

template <void (*act_func)(const NVTETensor, const NVTETensor, NVTETensor, musaStream_t)>
py::object dactivation_helper(const at::Tensor& grad, const at::Tensor& input,
  ...
  act_func(te_grad.data(), te_input.data(), te_output.data(), at::musa::getCurrentMUSAStream());
