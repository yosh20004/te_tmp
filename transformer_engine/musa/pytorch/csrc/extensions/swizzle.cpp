...
void swizzle_scaling_factors(transformer_engine::TensorWrapper& input, bool rowwise) {
  ...
  auto options = at::TensorOptions().dtype(torch::kByte).device(torch::kPrivateUse1);
  ...
  // Launch kernel
  nvte_swizzle_scaling_factors(input_cu.data(), output_cu.data(), at::musa::getCurrentMUSAStream());
...
at::Tensor rowwise_swizzle(at::Tensor input, at::Tensor scale_inv) {
  ...
  auto options = at::TensorOptions().dtype(scale_inv.dtype()).device(torch::kPrivateUse1);
