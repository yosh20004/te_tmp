...
CommOverlapHelper::CommOverlapHelper(c10d::ProcessGroup *world_group,
  ...
  backend_is_mccl = true; // (backend != c10d::ProcessGroup::BackendType::MCCL);
...
void CommOverlapHelper::ub_allgather(void *globaldata, size_t globalbytes, void *localdata,
  ...
  auto localtmp = (backend_is_mccl)
      ? localtensor.to(c10::Device(c10::kPrivateUse1), false, false)
      : localtensor;
  auto globaltensor =
      torch::from_blob(globaldata, {static_cast<int64_t>(globalbytes / sizeof(uint8_t))},
                       at::device(torch::kCPU).dtype(torch::kUInt8));
  auto globaltmp = (backend_is_mccl)
      ? globaltensor.to(c10::Device(c10::kPrivateUse1), false, false)
      : globaltensor;
...
void CommOverlap::copy_into_buffer(py::handle input, py::handle quantizer, bool local_chunk) {
  ...
  c10::musa::MUSAStream stream_main = at::musa::getCurrentMUSAStream();
  NVTE_CHECK_CUDA(musaEventRecord(_start_d2dcopy, (musaStream_t)stream_main));
  NVTE_CHECK_CUDA(musaStreamWaitEvent((musaStream_t)_stream_comm, _start_d2dcopy, 0));
  NVTE_CHECK_CUDA(musaMemcpyAsync(ubuf_ptr, input_tensor.dptr(),
                                  input_tensor.numel() * input_tensor.element_size(),
                                  musaMemcpyDeviceToDevice, (musaStream_t)_stream_comm));
}
...
void CommOverlapP2P::copy_into_buffer(py::handle input, py::handle quantizer, bool local_chunk) {
  ...
  if (local_chunk) {
    ...
    if (input_tensor.element_size() != (int64_t)_ubuf.element_size())
      NVTE_ERROR("input data type does not match communication buffer!");
    NVTE_CHECK_CUDA(musaMemcpyAsync(_ubufs[_tp_id].dptr(), input_ptr,
                                    input_tensor.numel() * input_tensor.element_size(),
                                    musaMemcpyDeviceToDevice, (musaStream_t)stream_main));
    ...
    if (input_tensor.element_size() != (int64_t)_ubuf.element_size())
      NVTE_ERROR("input data type does not match communication buffer!");
    NVTE_CHECK_CUDA(musaMemcpyAsync(_ubuf.dptr(), input_ptr,
                                    input_tensor.numel() * input_tensor.element_size(),
                                    musaMemcpyDeviceToDevice, (musaStream_t)stream_main));
  }
}
