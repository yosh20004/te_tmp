...
if _musa() is not None and _musa().is_available():
    _musa().manual_seed(seed)


def _require_perf_env():
    if _musa() is None or not _musa().is_available():
...
def _benchmark_musa_kernel(fn: Callable[[], object], warmup: int = 20, iters: int = 100) -> float:
    start_event = _musa().Event(enable_timing=True)
    end_event = _musa().Event(enable_timing=True)
...
def _make_router_bias(num_experts: int) -> torch.Tensor:
    bias = torch.arange(num_experts, device="musa", dtype=torch.float32) * 0.1
...
def test_fused_topk_router_perf_against_torch(
    ...
    torch_ms = _benchmark_musa_kernel(
